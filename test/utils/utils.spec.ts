...
describe("Module Utils", () => {
    describe("getInstance", () => {
        ...
        it("Should load class properly", () => {
            let dummy = Utils.getInstance<any>("Dummy, ./test/utils/dummy/dummy")
            let data = dummy.getData();
            Chai.expect(data).eq(200)
        })

        it("Should load class with class extension", () => {
            let dummy = Utils.getInstance<any>("Dummy, ./test/utils/dummy/dummy.js")
            let data = dummy.getData();
            Chai.expect(data).eq(200)
        })
        ...
        it("Should load class with nested modules", () => {
            let dummy = Utils.getInstance<any>("ParentModule.InnerModule.Dummy, ./test/utils/dummy/dummy-with-module")
            let data = dummy.getData();
            Chai.expect(data).eq(200)
        })
