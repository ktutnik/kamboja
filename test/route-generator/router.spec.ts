...
describe("Router", () => {
    it("Should load routes from controllers properly", async () => {
        let test = new RouteGenerator(["test/route-generator/api", "test/route-generator/controller"], new DefaultIdentifierResolver())
    ...
    it("Should throw error when provided path not found", async () => {
        let test = new RouteGenerator(["test/fake/path", "test/route-generator/controller"], new DefaultIdentifierResolver())
        let thrown = false;
        try{
